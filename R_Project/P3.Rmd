...
Looking at the raw values. ◦dplyr::glimpse()
...
### Summary statistic for control variables - Table 3 in the paper
...
```{r}
Group_characteristics_1 <- patientStudy %>%
  select("Group","Age","Length_Parkinson","H_Y","Medication","Sex") %>%
  filter (Group == 1)

ggplot(Group_characteristics_1,aes(x = Age))+
  geom_histogram(binwidth = 5)+
  ggtitle("Distribution of Age in Parkinson group") +
  labs(x= "Age in years")

ggplot(Group_characteristics_1,aes(x = H_Y))+
  geom_histogram(binwidth = 0.5)+
ggtitle("Distribution of Stage of Parkinson - Hoehn and Yahr scale") +
  labs(x= "H_Y score")

ggplot(Group_characteristics_1,aes(x = Length_Parkinson))+
  geom_histogram(binwidth = 2)+
  ggtitle("Distribution of Length of Parkinson ") +
  labs(x= "Length of Parkinson in Years")

ggplot(Group_characteristics_1,aes(x = Medication))+
  geom_histogram(binwidth = 50)+
  ggtitle("Distribution of Medication Dose in Levodopa Equivalent ") +
  labs(x= "Medication Dose in Levodopa Equivalent")
...
ggplot(Group_characteristics,aes(x = Group,y = Age))+
  geom_boxplot()+
  ggtitle("Age distribution by group")
...
```
```{r}
ggplot(Group_characteristics,aes(x = Group,y = H_Y))+
  geom_boxplot()
```
...
```{r}
patientStudy%>%
  filter(Group ==1, !is.na(H_Y))%>%
  select("Age","Medication","Length_Parkinson","MMSE", "H_Y")%>%
  cor()
```

```{r}
ggplot(patientStudy, aes(x=Age, y=MMSE)) +
  geom_smooth(method = "lm", se = FALSE) +
  ggtitle("Age vs. MMSE in sample population") +
  labs(x= "Age in years" , y="MMSE")
```
```{r}
patientStudy %>% filter(Group==1)%>%
ggplot(aes(x=Age, y=H_Y)) +
  geom_smooth(method = "lm", se = FALSE) +
  ggtitle("Age vs. H_Y in PD Group") +
  labs(x= "Age in years" , y="H_Y")
```
```{r}
patientStudy %>% filter(Group==1)%>%
ggplot(aes(x=MMSE, y=H_Y)) +
  geom_smooth(method = "lm", se = FALSE) +
  ggtitle("MMSE vs H_Y in PD Group") +
  labs(x= "MMSE score" , y="H_Y score")
```

```{r}
patientStudy%>%
ggplot(aes(x=Age, y=H_Y,colour = factor(Group))) +
  geom_point(method = "lm", se = FALSE) +
  ggtitle("Age vs. Parkinson_stage by disease group") +
  labs(x= "Age in years" , y="H_Y score")
```
...
ggplot(patientStudy, aes(x=Age, y=Length_Parkinson)) +
  geom_point() +
  ggtitle("Age vs length of Parkinson") +
  labs(x= "Age in years") 
```
```{r}
library(ggbeeswarm)
...
ggplot(aes(x = Age, y = Length_Parkinson)) +
  ...
  ggtitle("Age vs length of Parkinson in Parkinson group")+
